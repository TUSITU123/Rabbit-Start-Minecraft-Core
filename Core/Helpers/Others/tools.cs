...
namespace System
{
    public sealed class Tools
    {
        ...
        private bool vp;
        private HttpHelper web = new HttpHelper();
        /// <summary>
        /// 修改游戏标题
        /// </summary>
        ...
        public async Task<bool> ForgeInstallation(string ForgePath, string version, string java)
        {
            ...
            if ((this.SLC.FileExist(ForgePath) == null) && new Unzip().UnZipFile(ForgePath, System.Directory.GetCurrentDirectory() + @"\SquareMinecraftLauncher\Forge\", out str))
            {
                ...
                if (this.SLC.FileExist(string.Concat(textArray1)) == null)
                {
                    ...
                    string IDversion = "";
                    foreach (GameInfoType version2 in this.GetAllTheExistingVersion())
                    {
                        if (version2.Version == version)
                        {
                            IDversion = version2.Id;
                        }
                    }
        ...
        /// </summary>
        /// <returns></returns>
        public GameInfoType[] GetAllTheExistingVersion()
        {
        ...
        public Skin GetAuthlib_Injector(string yggdrasilURL, string username, string password)
        {
            ...
            {
                throw new LauncherException("yggdrasil网址有误");
            }
            if (root == null)
            {
                throw new LauncherException("网络有问题");
            }
            if (root.accessToken == null)
            {
                throw new LauncherException(Regex.Unescape(JsonConvert.DeserializeObject<BlessingSkinError>(str).errorMessage));
            }
            Skin skin = new Skin
            {
        ...
        public async Task<bool> GetCompareForgeVersions(string version)
        {
            ...
            {
                throw new LauncherException("没有装Forge");
            }
            char[] separator = new char[] { '.' };
        ...
        {
            GameInfoType[] allTheExistingVersion = this.GetAllTheExistingVersion();
        ...
        public UnifiedPass GetUnifiedPass(string ID, string username, string password)
        {
            ...
            {
                throw new LauncherException(Regex.Unescape(root.errorMessage));
            }
        ...
        public Getlogin MinecraftLogin(string username, string password)
        {
            ...
            {
                throw new LauncherException("账号密码不得为空");
            }
            ...
            if ((str == null) || !(str != ""))
            {
                throw new LauncherException("请检查网络");
            }
            ...
            if (root.error == "ForbiddenOperationException")
            {
                ...
                {
                    throw new LauncherException("凭证错误");
                }
                if (root.errorMessage == "Invalid credentials. Invalid username or password.")
                {
                    throw new LauncherException("密码账户错误");
                }
            }
            if (root.error == "GoneException") throw new LauncherException("账户已迁移微软");
            throw new LauncherException(root.error);
        }
        ...
        public void UninstallTheExpansionPack(ExpansionPack ExpansionPack, string version, string java)
        {
            ...
            switch (ExpansionPack)
            {
                ...
                case ExpansionPack.Fabric:
                    ...
                    {
                        fabricUninstall abricUninstall = new fabricUninstall();
                        string Uninstall = abricUninstall.Uninstall(version);
